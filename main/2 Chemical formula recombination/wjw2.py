...
# 读取 Excel 文件，假设文件名为 'data.xlsx'，并且化学式在第一列
file_path = r'C:\Users\lhl\Desktop\Mr_Yang_paper\garnet_compsoition\class1\Filtered_chemical_formula_no_repeatable.xlsx'
df = pd.read_excel(file_path)

# 假设化学式在第一列
chemical_formulas = df.iloc[:, 1]
...
for position, elements in element_stats.items():
    for element, count in elements.items():
        export_data.append({'位置': position, '元素': element, '计数': count})

# 创建一个 DataFrame
export_df = pd.DataFrame(export_data)

# 尝试导出为 Excel 文件，保存到桌面
output_file_path = os.path.join(os.path.expanduser('~'), 'Desktop', f'element_statistics.xlsx')
try:
...
# 将组合结果导出到新的 Excel 文件
combinations_df = pd.DataFrame(combinations, columns=['新化学式'])
combinations_output_path = os.path.join(os.path.expanduser('~'), 'Desktop', f'new_chemical_formulas_.xlsx')
try:
